...
Процедура ГП_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Конструктор = Обработки.zdc_КонструкторФорм.Создать();
	Конструктор.Инициализировать(ЭтотОбъект);
	
	Если Конструктор.ЗапретДоработкиФормы() Тогда
		Возврат;
	КонецЕсли;
	
	// Создание реквизитов, элемента формы, команды и кнопки
	Конструктор
		.СоздатьРеквизит("ВыбранныеТипыСтрокой", "Строка", , "Выбранные типы строкой")
		.СоздатьРеквизит("ВыбранныеТипы", "ОписаниеТипов", , "Выбранные типы")
		.ИзменитьРеквизиты()
		
		.СоздатьПоле("ПолеВыбранныеТипы", , "ВыбранныеТипы")
		.УстановитьПараметрЭлемента("Заголовок", "Допустимые типы")
		.УстановитьПараметрЭлемента("Подсказка", "Выберите типы значений для обработки")
		.УстановитьДействиеЭлемента("ПриИзменении", "ГП_ВыбранныеТипыПриИзменении")
		
		.СоздатьКоманду("ВыполнитьСФильтром", "Выполнить с фильтром", "ГП_ВыполнитьСФильтром", "Выполнить проведение с применением выбранных типов")
		.СоздатьКнопкуФормы("ВыполнитьСФильтром", , "ВыполнитьСФильтром");
	
	// Установка ограничения типов - только проводимые документы
	Элементы.ПолеВыбранныеТипы.ОграничениеТипа = ГП_ОбщийМодуль.ПолучитьОграничениеТипаПроводимыхДокументов();
