...
#Вставка
	...
	ВыбранныеТипы = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураПараметров, "ВыбранныеТипы", "");
	ЕстьОтборПоТипам = НЕ (ВыбранныеТипы = ""
		ИЛИ (ТипЗнч(ВыбранныеТипы) = Тип("Массив") И ВыбранныеТипы.Количество() = 0));

	Если ЕстьОтборПоТипам Тогда
		Запрос.Текст = ГП_ПолучитьТекстЗапросаПоПервичнымДокументам(ВыбранныеТипы);
	Иначе
		Запрос.Текст = ПолучитьТекстЗапросаПоПервичнымДокументам();
	КонецЕсли;
...
Функция ГП_ПолучитьТекстЗапросаПоПервичнымДокументам(ВыбранныеМетаданные)
	...
	Для Каждого МетаданныеДокумента Из Метаданные.Документы Цикл
		...
		Если МетаданныеДокумента.Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить 
			И ВыбранныеМетаданные.Найти(Тип("ДокументСсылка." + МетаданныеДокумента.Имя)) <> Неопределено Тогда
