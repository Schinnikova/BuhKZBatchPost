...
Процедура ГП_ПерепроведениеДокументовПоРасписанию() Экспорт
   ...
   Выборка = Справочники.Организации.Выбрать();
	...
	ДатаНачала = РегистрыСведений.ГП_ПараметрыРегламентныхЗаданий.Получить().Дата;
	Если НЕ ЗначениеЗаполнено(ДатаНачала) Тогда
		ДатаНачала = ?(День(ТекущаяДата) > 10, НачалоМесяца(ТекущаяДата), ДобавитьМесяц(НачалоМесяца(ТекущаяДата), -1));
	КонецЕсли;
	...
	ПараметрыПерепроведения.Вставить("ВыбранныеТипы", "РеализацияТоваровУслуг;КомплектацияНоменклатуры;ПеремещениеТоваров;ВозвратТоваровОтПокупателя"); // Выбранные типы 
